...
class TestModule(utility.TestModuleBase):
    ...
    def _generate_pattern_port0(self, num_tc5, send_port, flows=[0, 3, 4]):
        ...
        PAD = (int((1542+7)/8) - int((84+7)/8) * 17) * 8
        ...
        self.crafter.set_frame(send_port, 17, frame3)
        flow_sz.append((flows[1], frame3.get_length()[0]))
        for i in range(num_tc5):
            self.crafter.set_frame(send_port, i + 18, frame4)
            flow_sz.append((flows[2], frame4.get_length()[0]))
        for i in range(8 - num_tc5):
            self.crafter.set_frame(send_port, i + 18 + num_tc5, frame4.nop())
            flow_sz.append((flows[2], frame4.get_length()[0]))

        # override table. We don't need to count flow_sz.
        if num_tc5 < 8:
            self.crafter.set_frame(send_port, 25, frame4.nop().additional_wait(PAD))
        else:
            self.crafter.set_frame(send_port, 25, frame4.additional_wait(PAD))

        self.crafter.set_frame(send_port, 26, eol_frame)
    ...
    def _generate_pattern_port1(self, num_tc5, port_gap, send_port, flows=[1, 2, 5]):
        ...
        for i in range(num_tc5):
            self.crafter.set_frame(send_port, i + 18, frame5)
            flow_sz.append((flows[2], frame5.get_length()[0]))
        for i in range(8 - num_tc5):
            self.crafter.set_frame(send_port, i + 18 + num_tc5, frame5.nop())
            flow_sz.append((flows[2], frame5.get_length()[0]))

        # override table. We don't need to count flow_sz.
        if num_tc5 < 8:
            self.crafter.set_frame(send_port, 24, frame5.nop().additional_wait(PAD))
        else:
            self.crafter.set_frame(send_port, 24, frame5.additional_wait(PAD))

        self.crafter.set_frame(send_port, 25, frame5.additional_wait(port_gap))
    ...
    def test(self, num_frames, num_tc5, port1_gap, port2_en, result_dir):
        ...
        # The number of frames = 17 + 1 + 8 = 26
