...
|------|----|--------|--------------|------|------|
| flow0 | 7 | 84   | 1542 * 8 | 1000 Mbps | Port 0 |
| flow1 | 5 | 1542 | 1542 * 8 | 1000 Mbps | Port 1 |
...
### port1_gap
- The input sequence for port 1 is larger than the input sequence for port 0 by the number of bytes specified by the `--port1_gap` argument.
...
- Each time `port1_gap` increases by 8 bytes, the sequence of port 1 increases by 1 cycle.
    - We recommend setting `port1_gap` in 8-byte increments because if it is less than 8 bytes, the sequence will increase by either 0 or 1 cycle depending on the frame size.
- The initial value of `152` corresponds to a phase shift of 152 / 8 = 19 cycles (121.6 ns) each sequence.
...
```
$ python3 eval.py --efcc-jtag-target <N> --switch-jtag-target <M> --num_frames 30000 --port1_gap 152 --flow1_offset 72
...
Frame0139.ETHER(dst=3, src=1).IPV4(dst=8, src=6).VLAN(id=2, pcp=3).UDP(dst=5201, src=1234).Payload(length=14).AdditionalWait(32)
Frame0140.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0141.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0142.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0143.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0144.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0145.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
Frame0146.ETHER(dst=3, src=1).IPV4(dst=3, src=1).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=1472).Nop()
...
Frame0080.ETHER(dst=3, src=2).IPV4(dst=3, src=2).VLAN(id=4, pcp=1).UDP(dst=5201, src=1234).Payload(length=82).Nop()
EOL()
read_timestamp: |██████████████████████████████| 30000 / 30000
read_timestamp: |██████████████████████████████| 30000 / 30000
read_timestamp: |██████████████████████████████| 30000 / 30000
read_timestamp: |██████████████████████████████| 148 / 148
read_timestamp: |██████████████████████████████| 148 / 148
read_timestamp: |██████████████████████████████| 148 / 148
```
...
2.406,12.294,9.888,4.619,2.557,2.413,3.648,6.118,8.589,11.270
2.406,11.117,8.710,4.167,2.291,2.406,2.835,5.526,7.821,10.641
```
...
The below figure illustrates the latency of each frame for one sequence (148 frames) under the condition that the heads of flow 0 and flow 1 are aligned.  
