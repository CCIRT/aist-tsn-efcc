...
def plot_latencies(latencies, filename):
    ...
    for i, _ in enumerate(x):
        plt.text(x[i], y_min[i] + 0.2, f'{y_min[i]:.3f}')
        plt.text(x[i], y_avg[i] + 0.2, f'{y_avg[i]:.3f}')
        plt.text(x[i], y_max[i] + 0.2, f'{y_max[i]:.3f}')
...
if __name__ == '__main__':
    ...
    plot_latency_points_all(timestamps, recv_timestamps, 156.25, f'{args.result_dir}/latency_plot_all.png')
    plot_latency_points_all(timestamps2, recv_timestamps2, 156.25, f'{args.result_dir}/latency_plot_no_shift.png')

    print('----------------------')
    print('Plot latency of all frames')
    print('min,max,max-min,avg,std,25%,50%,75%,90%,99%')
    for flow, ts in timestamps.items():
        plot_latency_points(ts, 156.25, f'{args.result_dir}/latency_plot_{flow}.png')
